  ...
  function fentonBuildChart(R) {
    ...
    var panels = TABS.map(function (t, i) {
      return '<div id="' + grpId + 'p' + i + '" style="display:' + (i === 0 ? 'block' : 'none') + '">' +
        '<canvas id="' + grpId + 'c' + i + '" height="270"' +
        ' style="width:100%;max-height:290px;display:block"></canvas>' +
        '</div>';
    }).join('');
    ...
    // Push render job onto the queue — will be executed after innerHTML flush
    var renderJob = (function (sex, gaDec, tabs, grpId, patColFn) {
      return function () {
        var tables  = FENTON_LMS[sex];
        var gaSteps = xRange(22, 43, 0.5);
        var pc2     = patColFn(sex);

        tabs.forEach(function (t, i) {
          var canvasId = grpId + 'c' + i;
          var canvas   = document.getElementById(canvasId);
          if (!canvas) return;
          destroyChart(canvasId);

          var table = tables[t.key];
          if (!table) return;

          var datasets = pctDatasets(table, gaSteps, TEAL);

          if (t.patVal !== null) {
            var disp    = t.unit === 'g' ? t.patVal.toFixed(0) : t.patVal.toFixed(1);
            var pctDisp = t.patP !== null ? t.patP.toFixed(1) + 'th %ile' : '?';
            datasets.push({
              label: 'Patient · ' + disp + ' ' + t.unit + ' · ' + pctDisp,
              data: [{ x: gaDec, y: t.patVal }],
          ...
          new Chart(canvas, {
            ...
            options: makeOpts(22, 43, 2, t.yMin, t.yMax, t.yStep,
              'Gestational Age (weeks)', t.yLabel, 'GA'),
          });
        });
      };
    })(
      ...
      TABS.map(function (t) {
        return {
          ...
          yStep:  t.yStep,
          yLabel: t.yLabel,
          patVal: t.result ? t.result.value : null,
          patP:   t.result ? t.result.p     : null,
        };
      }),
      grpId,
      patCol
  ...
  global._gcFentonTab = function (grpId, active) {
    for (var i = 0; i < 3; i++) {
      ...
      if (btn) {
        ...
        // Re-render chart on tab show (canvas may have been hidden at render time)
        if (i === active) {
          var canvas = document.getElementById(grpId + 'c' + i);
          if (canvas) {
  ...
  function _installPatches() {
    ...
    if (typeof origFenton === 'function') {
      global.fentonRenderResults = function (R) {
        ...
        // fentonBuildChart was called inside origFenton and pushed to queue
        flushQueue();
      };
    }
