 ...
 * ─────────────────────────────────────────────────────────────────────────
 * Features:
 *   • Dynamic ingredient rows with Malawi FCT + CalorieNinjas lookup
...
(function (global) {
  ...
  // ── Version helper ───────────────────────────────────────────────────────
  // Single source of truth: reads APP_VERSION set by index.html <script>.
  // Falls back to the <meta name="app-version"> tag, then to a safe default.
  function _getAppVersion() {
    if (typeof APP_VERSION !== 'undefined' && APP_VERSION) return APP_VERSION;
    try {
      const meta = document.getElementById('meta-app-version');
      if (meta) {
  ...
  async function _lookupNutrition(query) {
    ...
    // 2. NTFoodSearch (FDC → CalorieNinjas)
  ...
  function _rcShowDropdown(id, items) {
    ...
    items.forEach(function (item, idx) {
      ...
      const srcColor = item.source === 'Malawi FCT' ? '#34d399' :
                       item.source === 'CaloriesNinja' ? '#fb923c' : '#60a5fa';
      row.style.cssText = [
        'display:flex',
        'align-items:center',
  ...
  function _renderNutriBadge(ing) {
    ...
    const srcColor = ing.source === 'Malawi FCT' ? '#34d399' :
                     ing.source === 'CaloriesNinja' ? '#fb923c' : '#60a5fa';
  ...
  function _generate() {
    ...
    if (!_state.userName.trim()) {
      _showMsg('rc-gen-msg', '⚠ Enter your name before generating.', 'warn'); return;
    }
    ...
    card.innerHTML = `
      ...
      <div style="background:linear-gradient(135deg,rgba(29,233,212,0.12),rgba(96,165,250,0.08));
        ...
        <div style="display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:10px">
          ...
          <div style="text-align:right;flex-shrink:0">
            ...
            <div style="font-family:var(--mono);font-size:11px;font-weight:700;color:var(--text-bright)">${_escHtml(_state.userName)}</div>
      ...
      <div style="font-family:var(--mono);font-size:8.5px;color:var(--text-muted);line-height:1.8;
        ...
        Data sources: Malawi Food Composition Table (2019) · USDA FoodData Central · CalorieNinjas API<br>
        For clinical use, verify values against original FCT and patient-specific requirements.<br>
        Recipe Calculator • Version ${_getAppVersion()} — © 2026 All Rights Reserved
      </div>
    `;
  ...
  function init() {
    _load();
    // Auto-fill name from profile if not already saved in recipe state
    if (!_state.userName) {
      try {
        // Prefer getUserProfile() (nt_user_profile) then currentSettings
        const profileName =
    ...
    // Sync version footer element — reads from APP_VERSION (single source of truth)
    try {
      const footerVer = document.getElementById('rc-footer-version');
      if (footerVer) {
        footerVer.textContent = 'Recipe Calculator  •  Version ' + _getAppVersion();
      }
    } catch (_) {}
