...
- When answering questions about Oasis CNST itself (features, modules, purpose, developer, methodology, limitations), use ONLY the official About-section EVIDENCE BLOCK. If not documented there, say so per Guardrail 5.`;
  ...
  const _RAGLayer = {
     ...
     */
    async fetchContext(query, context = 'both', topK = 7) {
      try {
  ...
  const _GuardrailRouter = {
     ...
     *
     * Only the About-section and live-Enteral-calculator-state matches
     * are treated as "authoritative" (self-contained, safe to answer
     * from without going further). A FoodDB/DNI/RefDB hit alone does
     * NOT mark Layer 1 sufficient — those are often partial/incidental
     * keyword matches (e.g. a query like "sports nutrition" tokenizing
     * against unrelated food names), and per the spec, food/nutrient
     * values should prefer the live Chakudya API (Layer 2) anyway. So
     * those blocks are still collected and passed along as evidence,
     * but Layer 2/3 are still consulted rather than short-circuited.
     */
    gatherLayer1(userMessage) {
      const blocks = [];
      const sources = [];
      ...
      if (_AboutDB.detectQuery(userMessage)) {
        ...
        if (ctx) { blocks.push(ctx); sources.push('Oasis CNST About Section'); authoritative = true; }
      }
      if (_EnteralCalcDB.detectQuery(userMessage)) {
        const ctx = _EnteralCalcDB.buildContext(_EnteralCalcDB.getState());
        if (ctx) { blocks.push(ctx); sources.push('Oasis Enteral Calculator (live state)'); authoritative = true; }
      }
    ...
    async route(userMessage) {
      ...
      if (!l1.sufficient) {
        // Layer 1 alone isn't an authoritative closed answer (only About /
        // live-Enteral-state matches count as that) — consult Layer 2
        // (Chakudya structured API) and Layer 3 (Chakudya RAG) together,
        // in parallel, rather than gating RAG on the live-DB result.
        [l2, l3] = await Promise.all([
          this.gatherLayer2(userMessage),
          this.gatherLayer3(userMessage),
        ]);
      } else if (needDeep) {
        // Layer 1 was authoritative, but the query still signals a need
        // for deeper guideline-level reasoning — consult RAG on top.
...
PES STATEMENT: [Full integrated PES in one sentence]`;
    ...
    try {
      const ragCtx = await _RAGLayer.fetchContext(
        `PES statement ${diagnosis} ${etiology} nutrition diagnosis`, 'both', 5
      );
      if (ragCtx) pesSystem += '\n\n' + ragCtx;
    } catch (_) {}
...
Be specific, concise, and clinically relevant. Use eNCPT language.`;
    ...
    try {
      const ragCtx = await _RAGLayer.fetchContext(
        `ADIME nutrition note ${diagnosis} ${assessment} intervention monitoring`, 'both', 5
      );
...
MONITORING PARAMETERS: [≤3 key parameters]`;
    ...
    try {
      ...
      const ragCtx = await _RAGLayer.fetchContext(ragQuery, 'both', 6);
...
Keep it under 200 words. Use professional clinical language.`;
    ...
    try {
      ...
      const ragCtx = await _RAGLayer.fetchContext(summaryQuery, 'both', 4);
