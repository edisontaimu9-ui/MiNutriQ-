 ...
 *  • Firebase Storage for file uploads (max 25 MB), with live progress
 *  • Duplicate upload guard (per-user, by title)
 *  • view / download tracking counters (atomic Firestore increment)
 *  • Admin dashboard hooks: exposes window.LibraryAdminAPI
 *
 * Storage / Database:
 *  • Firebase Storage folder  : "library/{uid}/{filename}"
 *  • Firestore collection     : "library_resources"  (LIB_COL)
 *  • Bookmarks                : localStorage  key "oasis_lib_bm_{uid}"
 *
 * Uses the same Firebase app/config already initialised elsewhere in the
 * app (see js/modules/firebase-rtdb-tabs.js) — no separate client needed.
 *
 * Integration:
 *  1. Ensure firebase-storage-compat.js and firebase-firestore-compat.js
 *     are loaded before this file (already the case in index.html)
 *  2. Add <script src="js/library.js"></script> before </body> in index.html
 *
 * Firestore document schema (library_resources collection):
 *  {
 ...
 *    fileURL       : string           // Firebase Storage download URL (empty for links)
...
(function () {
  ...
  /* ════════════════════════════════════════════════════
     CONSTANTS
  ════════════════════════════════════════════════════ */
  var LIB_VERSION       = '1.5.0';   // rewired: Appwrite → Firebase (Firestore + Storage)
  /* Firestore collection + Storage folder — Appwrite retired, back on Firebase
     (same project/config already loaded by firebase-rtdb-tabs.js).            */
  var LIB_COL     = 'library_resources';
  var LIB_STORAGE_FOLDER = 'library';
  var LIB_MAX_MB  = 25;
  ...
  var _unsubMine     = null;    // Firestore onSnapshot unsubscribe fn — my uploads
  var _unsubBrowse   = null;    // Firestore onSnapshot unsubscribe fn — local library/approved list
  var _initDone      = false;
  ...
  function _injectCSS() {
    ...
    s.textContent = [
      ...
      '.lib-hist-lbl{flex-shrink:0;font-family:var(--mono);font-size:8.5px;font-weight:700;',
      ...
      '.lib-hist-chip{flex-shrink:0;padding:3px 10px;border-radius:100px;font-family:var(--mono);',
        'font-size:9px;background:var(--surface2);border:1px solid var(--border);color:var(--text-dim);',
      ...
      '.lib-hist-clear{flex-shrink:0;margin-left:auto;padding:3px 8px;border-radius:6px;',
        'font-family:var(--mono);font-size:8.5px;background:none;border:1px solid transparent;',
  ...
  /* ════════════════════════════════════════════════════
     FIREBASE ACCESSORS  (Firestore + Storage)
  ════════════════════════════════════════════════════ */

  /** Return Firestore instance, or null if the SDK/app isn't ready yet. */
  function _fbDb() {
    try {
  ...
  /** Return Firebase Storage instance, or null if the SDK/app isn't ready yet. */
  function _fbStor() {
    try {
      if (typeof firebase === 'undefined' || !firebase.apps || !firebase.apps.length) return null;
  ...
  /**
   * Normalise a Firestore document snapshot to the shape expected by all
   * existing render/logic code. Maps snapshot.id → id and merges the data
   * so that _fmtDate(), _getYear(), and card templates work unchanged.
   * @param {firebase.firestore.DocumentSnapshot} snap
   * @returns {Object}
   */
  function _fbNormDoc(snap) {
    var data = snap.data() || {};
    return Object.assign({}, data, {
      id:         snap.id,
      uploadedAt: data.createdAt   // _fmtDate() already handles ISO strings
    });
  }
  ...
  /* Timestamps use new Date().toISOString(); counters use
     firebase.firestore.FieldValue.increment() for atomic writes. */
  ...
  function _cardHTML(r, showStatus, showOasisBadge) {
    ...
    var hasDl = !!(r.fileURL||r.externalLink);
  ...
  /**
   * Subscribe to Firestore realtime updates on the approved-resources query.
   * Uses docChanges() so we only react to what actually changed:
   *   • removed — remove from list (deleted, or status flipped away from approved)
   *   • added   — insert if not already present
   *   • modified — refresh its entry in cache and re-render
   *
  ...
  function _subscribeApproved() {
    ...
    var query = db.collection(LIB_COL)
      .where('status', '==', 'approved')
      .orderBy('createdAt', 'desc')
      .limit(120);

    _unsubBrowse = query.onSnapshot(function(snapshot) {
      var changed = false;
      snapshot.docChanges().forEach(function(change) {
        var normalised = _fbNormDoc(change.doc);
        ...
        if (change.type === 'added') {
          if (idx === -1) { _resources.unshift(normalised); changed = true; }
        } else if (change.type === 'modified') {
          if (idx !== -1) { _resources[idx] = normalised; }
          else { _resources.unshift(normalised); }
          changed = true;
        }
      });
      if (changed) {
      ...
      console.error('[Library] approved subscription err:', e);
    });

    console.log('[Library] Firestore browse subscription active');
  }

  /* ════════════════════════════════════════════════════
     DATA: LOAD APPROVED  (Firestore)
  ════════════════════════════════════════════════════ */
  function _loadApproved() {
    var db = _fbDb();
    if (!db) { _renderLocal(); return; }
    var cont = document.getElementById('lib-local-cards');
    if (cont) cont.innerHTML = '<div class="lib-spin"></div>';
    db.collection(LIB_COL)
      .where('status', '==', 'approved')
      .orderBy('createdAt', 'desc')
      .limit(120)
      .get()
      .then(function(snapshot) {
        _resources = snapshot.docs.map(_fbNormDoc);
        _sqClearCache();   // invalidate scored search cache after fresh data
        _renderLocal();
        _renderBookmarks();
        _subscribeApproved();   // ← keep browse panel in sync with Firestore realtime
      }).catch(function(e) {
  ...
  /* ════════════════════════════════════════════════════
     DATA: MY UPLOADS  (Firestore + realtime)
  ════════════════════════════════════════════════════ */
  function _subscribeMyUploads() {
    var db  = _fbDb();
    var uid = _curUser() && _curUser().uid;
    if (!db || !uid) { _renderMine(); return; }
    ...
    _unsubMine = query.onSnapshot(function(snapshot) {
      _myResources = snapshot.docs.map(_fbNormDoc);

      /* Detect admin status changes on this refresh and surface toast + nav dot */
      _myResources.forEach(function(r) {
        var prev = prevStatuses[r.id];
        if (prev && prev !== r.status && r.status !== 'pending') {
          var msg = r.status === 'approved'
            ? '✓ "' + r.title + '" was approved'
            : '✗ "' + r.title + '" was not approved';
      ...
      console.error('[Library] myUploads err:', e);
      _renderMine();
    });
  }

  /* ════════════════════════════════════════════════════
     DATA: BOOKMARKS  (localStorage — no Firestore collection)
  ...
  function _openResource(resourceId) {
    ...
    // Metadata pane
    var _actUrl = r.fileURL || r.externalLink || '';
  ...
  /* ── View / download counter — atomic Firestore increment ── */
  function _trackView(resourceId, action) {
    var db = _fbDb();
    if (!db) return;
    var field = action === 'view' ? 'viewCount' : 'downloadCount';
    var patch = {};
    patch[field] = firebase.firestore.FieldValue.increment(1);
    db.collection(LIB_COL).doc(resourceId).update(patch)
  ...
  /* ════════════════════════════════════════════════════
     UPLOAD — EXECUTE  (Firebase Storage + Firestore)
  ════════════════════════════════════════════════════ */
  function _doUpload(title, desc, category, source, type, linkUrl) {
    ...
    /* ── Create Firestore document after file is (optionally) uploaded ── */
    function _saveDoc() {
      if (progLbl) progLbl.textContent = 'Saving metadata…';
      if (progBar) progBar.style.width = '95%';
      return db.collection(LIB_COL).add({
        title:         title,
        ...
        source:        source,
        fileType:      type,
        /* Firebase Storage download URL (empty for external links) */
        fileURL:       fileURL,
    ...
    if (!db) {
      _toast('Database not available — reload and try again', 'error');
      _onErr(new Error('No Firestore instance'));
      return;
    }
    ...
    if (type === 'link') {
      ...
      /* File upload → Firebase Storage (with real progress), then save metadata doc */
      if (!stor) {
        _toast('Storage not available — reload and try again', 'error');
        _onErr(new Error('No Firebase Storage'));
        return;
      }
      if (progBar) progBar.style.width = '2%';
      if (progLbl) progLbl.textContent = 'Uploading…';

      var uniqueName = Date.now() + '_' + Math.random().toString(36).slice(2, 8) +
                        '_' + _selectedFile.name;
      var ref  = stor.ref().child(LIB_STORAGE_FOLDER + '/' + user.uid + '/' + uniqueName);
      var task = ref.put(_selectedFile);

      task.on('state_changed', function(snap) {
        var pct = snap.totalBytes ? Math.round((snap.bytesTransferred / snap.totalBytes) * 90) : 0;
  ...
  /* ════════════════════════════════════════════════════
     ...
     All operations use Firestore + Firebase Storage.
  ════════════════════════════════════════════════════ */
  window.LibraryAdminAPI = {
    /**
     * Approve a resource.
     * @param {string} resourceId — Firestore document id
     * @param {string} [note]      optional admin note
     */
    approve: function(resourceId, note) {
      var db = _fbDb();
      if (!db) return Promise.reject('No Firestore instance');
      return db.collection(LIB_COL).doc(resourceId).update({
        status:     'approved',
        reviewNote: note || '',
    ...
    update: function(resourceId, fields) {
      ...
      return db.collection(LIB_COL).doc(resourceId).update(safe);
    },

    /**
     * Delete a resource and its Firebase Storage file.
     * @param {string} resourceId
     */
    delete: function(resourceId) {
      ...
      var stor = _fbStor();
      if (!db) return Promise.reject('No Firestore instance');
      var docRef = db.collection(LIB_COL).doc(resourceId);
      return docRef.get().then(function(snap) {
        var data = snap.data() || {};
        var del  = docRef.delete();
        /* Best-effort: delete file from Storage — ignore errors */
        if (data.fileURL && stor) {
          stor.refFromURL(data.fileURL).delete().catch(function(){});
        }
        return del;
      });
    },
    ...
    fetchByStatus: function(status) {
      var db = _fbDb();
      if (!db) return Promise.reject('No Firestore instance');
      var query = db.collection(LIB_COL).orderBy('createdAt', 'desc').limit(300);
      if (status && status !== 'all') {
        query = query.where('status', '==', status);
      }
      return query.get().then(function(snapshot) {
        return snapshot.docs.map(_fbNormDoc);
      });
    },
