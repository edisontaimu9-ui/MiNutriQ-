 ...
 *  • Local Library — search & browse Oasis-stored resources only, with its
 *    own search bar, category/type filters, and search history
 *  • Web Library — search & browse internet sources only (WHO, ASPEN, ESPEN,
 *    NICE, Cochrane/PubMed, OpenAlex, Frontiers, Elsevier), with its own
 *    search bar, year/open-access/Malawi filters, and search history
...
(function () {
  ...
  /* ════════════════════════════════════════════════════
     CONSTANTS
  ════════════════════════════════════════════════════ */
  var LIB_VERSION       = '1.4.0';   // bumped: split Browse into Local Library + Web Library
  ...
  var _panel         = 'local'; // 'local' | 'web' | 'upload' | 'myuploads' | 'bookmarks'
  var _filterCat     = '';      // Local Library: category filter
  var _filterType    = '';      // Local Library: file-type filter
  var _searchQ       = '';      // Local Library: search query
  var _webQ          = '';      // Web Library: search query
  var _webMalawiOnly = false;   // Web Library: restrict to Malawi/Africa institutional sources
  var _activeRes     = null;
  var _citStyle      = 'apa';
  var _uploadTags    = [];
  var _selectedFile  = null;
  var _unsubMine     = null;    // Appwrite Realtime unsubscribe fn — my uploads
  var _unsubBrowse   = null;    // Appwrite Realtime unsubscribe fn — local library/approved list
  var _initDone      = false;

  /* ── Search history (per-section, persisted to localStorage) ── */
  var LIB_HIST_MAX      = 8;
  var LIB_HIST_KEY_LOC  = 'oasis_lib_search_history_local';
  var LIB_HIST_KEY_WEB  = 'oasis_lib_search_history_web';
  var _localHistory   = [];
  var _webHistory     = [];
  ...
  function _injectCSS() {
    ...
    s.textContent = [
      ...
      '.lib-sntab{flex-shrink:0;padding:13px 18px;font-family:var(--mono);font-size:11px;font-weight:700;',
      ...
      '.lib-sec-title{font-family:var(--mono);font-size:11px;font-weight:700;letter-spacing:1px;',
        'text-transform:uppercase;color:var(--text-dim)}',
      '.lib-badge{background:rgba(29,233,212,.1);border:1px solid rgba(29,233,212,.2);border-radius:100px;',
        'padding:2px 9px;font-family:var(--mono);font-size:11px;font-weight:700;color:var(--teal)}',
      ...
      '.lib-search-btn{padding:6px 14px;background:var(--teal);border:none;border-radius:7px;',
        'font-family:var(--mono);font-size:11px;font-weight:700;color:#020617;cursor:pointer;',
      ...
      '.lib-aio-label{font-family:var(--mono);font-size:11px;font-weight:700;letter-spacing:1.2px;color:rgba(29,233,212,0.8);text-transform:uppercase}',
      ...
      '.lib-aio-loading{display:flex;align-items:center;gap:8px;padding:10px 14px 12px;font-family:var(--mono);font-size:11px;color:rgba(255,255,255,0.35);letter-spacing:.5px}',
      ...
      '.lib-aio-sources-toggle{display:flex;align-items:center;gap:6px;padding:5px 14px 10px;cursor:pointer;font-family:var(--mono);font-size:11px;color:rgba(255,255,255,0.35);letter-spacing:.5px;transition:color .15s;width:100%;background:none;border:none;text-align:left}',
      ...
      '.lib-aio-src-badge{flex-shrink:0;font-family:var(--mono);font-size:11px;font-weight:700;padding:2px 7px;border-radius:100px;letter-spacing:.5px;white-space:nowrap}',
      ...
      '.lib-aio-err{padding:8px 14px 12px;font-family:var(--mono);font-size:11px;color:rgba(248,113,113,0.6);letter-spacing:.5px}',
      ...
      '.lib-chip{flex-shrink:0;padding:4px 11px;border-radius:100px;font-family:var(--mono);font-size:11px;',
      ...
      '.lib-hist-row{display:flex;align-items:center;gap:6px;overflow-x:auto;scrollbar-width:none;',
        'margin:-2px 0 12px;padding-bottom:1px}',
      '.lib-hist-row::-webkit-scrollbar{display:none}',
      '.lib-hist-lbl{flex-shrink:0;font-family:var(--mono);font-size:11px;font-weight:700;',
        'letter-spacing:.6px;text-transform:uppercase;color:var(--text-muted)}',
      '.lib-hist-chip{flex-shrink:0;padding:3px 10px;border-radius:100px;font-family:var(--mono);',
        'font-size:11px;background:var(--surface2);border:1px solid var(--border);color:var(--text-dim);',
        'cursor:pointer;transition:all .15s;white-space:nowrap}',
      '.lib-hist-chip:hover{border-color:var(--teal);color:var(--teal)}',
      '.lib-hist-clear{flex-shrink:0;margin-left:auto;padding:3px 8px;border-radius:6px;',
        'font-family:var(--mono);font-size:11px;background:none;border:1px solid transparent;',
        'color:var(--text-muted);cursor:pointer;opacity:.7;transition:opacity .15s,color .15s}',
      '.lib-hist-clear:hover{opacity:1;color:#f87171}',
      ...
      '.lib-card-desc{font-family:var(--mono);font-size:11px;color:var(--text-dim);line-height:1.55;',
      ...
      '.lib-tag{font-family:var(--mono);font-size:11px;padding:2px 7px;border-radius:100px;',
      ...
      '.lib-card-cat{font-family:var(--mono);font-size:11px;color:var(--text-muted);',
      ...
      '.lib-cta-btn{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;',
        ...
        'cursor:pointer;font-family:var(--mono);font-size:11px;font-weight:700;letter-spacing:.6px;',
      ...
      /* Text label */
      '.lib-cta-label{font-size:11px;line-height:1;letter-spacing:.7px;',
      ...
      '.lib-lbl{display:block;font-family:var(--mono);font-size:11px;font-weight:700;letter-spacing:.8px;',
      ...
      '.lib-drop-sub{font-family:var(--mono);font-size:11px;color:var(--text-muted)}',
      ...
      '.lib-fp-size{font-family:var(--mono);font-size:11px;color:var(--text-muted);flex-shrink:0}',
      ...
      '.lib-review-note{font-family:var(--mono);font-size:11px;color:var(--text-dim);line-height:1.55;',
      ...
      '.lib-cit-tab{padding:4px 13px;font-family:var(--mono);font-size:11px;font-weight:700;',
      ...
      '.lib-cit-block{position:relative;background:var(--surface2);border:1px solid var(--border);',
        'border-radius:8px;padding:10px 60px 10px 12px;font-family:var(--mono);font-size:11px;',
      ...
      '.lib-cit-copy{position:absolute;top:7px;right:8px;background:var(--surface);',
        ...
        'font-family:var(--mono);font-size:11px;color:var(--teal);cursor:pointer;transition:all .15s}',
      '.lib-cit-copy:hover{background:rgba(29,233,212,.1)}',

      /* ── Related resources ── */
      '.lib-rel-hdr{font-family:var(--mono);font-size:11px;font-weight:700;letter-spacing:.9px;',
      ...
      '.lib-rel-item{display:flex;align-items:center;gap:8px;padding:7px 10px;background:var(--surface2);',
        ...
        'font-family:var(--mono);font-size:11px;color:var(--text-dim);margin-bottom:5px}',
      ...
      '.lib-act-link{display:inline-flex;align-items:center;gap:5px;margin-top:8px;',
        'padding:7px 14px;border-radius:7px;font-family:var(--mono);font-size:11px;font-weight:700;',
      ...
      '.rs-stab{flex:1;padding:7px 10px;background:var(--surface2);border:1px solid var(--border);',
        'border-radius:8px;font-family:var(--mono);font-size:11px;font-weight:700;letter-spacing:.5px;',
      ...
      '.rs-result-authors{font-family:var(--mono);font-size:11px;color:var(--text-dim);margin-bottom:5px;',
        'white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
      '.rs-result-meta{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:7px}',
      '.rs-result-journal{font-family:var(--mono);font-size:11px;color:var(--teal);',
      ...
      '.rs-result-year{font-family:var(--mono);font-size:11px;color:var(--text-muted)}',
      '.rs-result-cite{font-family:var(--mono);font-size:11px;color:var(--text-muted)}',
      '.rs-result-abstract{font-family:var(--mono);font-size:11px;color:var(--text-dim);line-height:1.6;',
      ...
      '.rs-source-badge{font-family:var(--mono);font-size:11px;font-weight:700;letter-spacing:.5px;',
      ...
      '.rs-filter-sel{background:var(--surface2);border:1px solid var(--border);border-radius:7px;',
        'padding:7px 10px;font-family:var(--mono);font-size:11px;color:var(--text-dim);',
        'outline:none;cursor:pointer;flex:1;min-width:120px}',
      '.rs-filter-sel:focus{border-color:rgba(29,233,212,.4)}',
      '.rs-sort-info{font-family:var(--mono);font-size:11px;color:var(--text-muted);',
      ...
      '.rs-pg-btn{padding:6px 16px;background:var(--surface2);border:1px solid var(--border);',
        'border-radius:7px;font-family:var(--mono);font-size:11px;color:var(--text-dim);',
        'cursor:pointer;transition:all .15s}',
      '.rs-pg-btn:hover:not(:disabled){border-color:var(--teal);color:var(--teal)}',
      '.rs-pg-btn:disabled{opacity:.35;cursor:not-allowed}',
      '.rs-pg-info{font-family:var(--mono);font-size:11px;color:var(--text-dim)}',

      /* ── Layer selector tabs (kept for theme compat) ── */
      /* ── Mixed results divider ── */
      '.lib-src-divider{font-family:var(--mono);font-size:11px;font-weight:700;letter-spacing:.8px;',
      ...
      '.lib-load-more{width:100%;padding:10px;margin-top:8px;background:var(--surface2);',
        ...
        'font-size:11px;color:var(--text-dim);cursor:pointer;transition:all .15s}',
  ...
  /* ── Precise client-side year-range check ──────────────────────────────
     Used as a guaranteed filter on top of (or instead of) server-side date
     params, since not every source (Frontiers, Elsevier) supports/honours
     a year filter server-side. A result with no parsable year is excluded
     whenever a year filter is active — we can't confirm it's in range, so
     "precise" means we don't let unknown-year items slip through. */
  function _yearInRange(yearStr, yearFrom, yearTo) {
    if (!yearFrom && !yearTo) return true;          // no filter active
    var y = parseInt(yearStr, 10);
    if (!y || isNaN(y)) return false;                // unknown year — exclude under an active filter
  ...
  /* ════════════════════════════════════════════════════
     SEARCH HISTORY  (per-section — Local Library / Web Library)
     Persisted independently to localStorage so each section
     remembers its own recent queries across sessions.
  ════════════════════════════════════════════════════ */
  function _histLoad(key) {
    try {
      var raw = localStorage.getItem(key);
      var arr = raw ? JSON.parse(raw) : [];
      return Array.isArray(arr) ? arr.filter(function(q){ return typeof q === 'string' && q; }) : [];
    } catch(e) { return []; }
  }

  function _histSave(key, arr) {
    try { localStorage.setItem(key, JSON.stringify(arr.slice(0, LIB_HIST_MAX))); } catch(e) { /* quota / private mode */ }
  }

  function _histClear(key) {
    try { localStorage.removeItem(key); } catch(e) {}
    return [];
  }

  /** Add a query to a history array (most-recent-first, deduped, capped). */
  function _histAdd(arr, key, q) {
    q = String(q || '').trim();
    if (!q || q.length < 2) return arr;
    var lower = q.toLowerCase();
    var next = [q].concat(arr.filter(function(h){ return h.toLowerCase() !== lower; }));
    if (next.length > LIB_HIST_MAX) next = next.slice(0, LIB_HIST_MAX);
    _histSave(key, next);
    return next;
  }

  /** Render a history chip row into the given container id. */
  function _renderHistChips(containerId, arr, runFnName, clearFnName) {
    var el = document.getElementById(containerId);
    if (!el) return;
    if (!arr.length) { el.innerHTML = ''; return; }
    var chips = arr.map(function(q) {
      var esc = _esc(q);
      var jsSafe = esc.replace(/'/g, "\\'");
      return '<button class="lib-hist-chip" onclick="LibraryModule.'+runFnName+'(\''+jsSafe+'\')">'+esc+'</button>';
    }).join('');
    el.innerHTML =
      '<div class="lib-hist-row">' +
        '<span class="lib-hist-lbl">Recent</span>' +
        chips +
        '<button class="lib-hist-clear" title="Clear search history" onclick="LibraryModule.'+clearFnName+'()">Clear</button>' +
      '</div>';
  }

  function _renderLocalHistory() { _renderHistChips('lib-local-hist', _localHistory, 'searchLocalFromHistory', 'clearLocalHistory'); }
  function _renderWebHistory()   { _renderHistChips('lib-web-hist',   _webHistory,   'searchWebFromHistory',   'clearWebHistory'); }
  ...
  function _aioFormat(text) {
    return text
      ...
      .replace(/\*(.+?)\*/g, '<em>$1</em>')
      .replace(/`(.+?)`/g, '<code style="font-family:var(--mono);font-size:11px;background:rgba(29,233,212,0.1);padding:1px 5px;border-radius:4px;color:rgba(29,233,212,0.9)">$1</code>')
    ...
    el.innerHTML =
      '<div class="lib-aio-card">' +
        '<div class="lib-aio-header">' +
          ...
          '<span style="font-family:var(--mono);font-size:11px;color:rgba(255,255,255,0.2);margin-left:auto;letter-spacing:.5px">Oasis AI · Groq</span>' +
        '</div>' +
      ...
      '<div class="lib-aio-card">' +
        ...
        (sourcesHtml
          ? '<button id="lib-aio-src-toggle" class="lib-aio-sources-toggle" onclick="_aioToggleSources()">' +
              ...
              '<span style="font-family:var(--mono);font-size:11px;margin-left:2px;opacity:.5">(' + sourceCount + ')</span>' +
            '</button>' +
  ...
  /* Main trigger — called from _renderWeb after results land */
  ...
  /* ─────────────────────────────────────────────────────────────────────────
     _renderLocal — Local Library section (Oasis-stored resources only)
     _renderWeb   — Web Library section (external/internet sources only)
  ───────────────────────────────────────────────────────────────────────── */
  function _renderLocal() {
    var cont  = document.getElementById('lib-local-cards');
    var badge = document.getElementById('lib-local-count');
    if (!cont) return;
    ...
    cont.innerHTML = filtered.length
      ...
      : _renderEmpty(_searchQ.trim()
          ? 'No matching resources found in the Oasis Library.<br>Try different keywords or filters.'
          : 'No approved resources found.<br>Try different filters or upload a resource.');
  }

  function _renderWeb() {
    var cont   = document.getElementById('lib-web-cards');
    var badge  = document.getElementById('lib-web-count');
    var label  = document.getElementById('lib-web-results-label');
    ...
    var isMalawiFilter = _webMalawiOnly;

    // When the Malawi/Africa filter is active, restrict GL results to the
    ...
    var totalVisible  = glResults.length + frResults.length + elResults.length + extResults.length;

    if (label) label.textContent = _webQ.trim() ? 'Search Results' : 'Web Sources';
    ...
    if (!html) {
      html = _bgLoading
        ...
        : (_webQ.trim()
            ...
            : _renderEmpty('Search WHO, ASPEN, ESPEN, NICE, Cochrane/PubMed, OpenAlex, Frontiers &amp; Elsevier.<br>Enter a term above to begin.'));
    }
    ...
    if (!_bgLoading && _webQ && _webQ.trim().length >= 3) {
      _triggerAIOverview(_webQ, [], glResults, frResults, elResults, extResults);
    }
  }
  ...
  /* ── Local Library search — synchronous, local data only, no network ────── */
  function _localSearch(q) {
    _searchQ = q || '';
    _renderLocal();
  }

  /** Explicit submit (Enter key / Search button) — records search history. */
  function _submitLocalSearch(q) {
    _localSearch(q);
    if (_searchQ.trim().length >= 2) {
      _localHistory = _histAdd(_localHistory, LIB_HIST_KEY_LOC, _searchQ.trim());
      _renderLocalHistory();
    }
  }

  /* ── Web Library search — debounced at 400 ms to reduce excessive API calls ─ */
  var _bgSearchTimer = null;
  function _webSearch(q) {
    _webQ = q || '';
    clearTimeout(_bgSearchTimer);

    if (_webQ.trim().length < 2) {
      _doWebSearch(_webQ);
      return;
    }

    // Show a "searching…" state immediately while debouncing the API calls
    _bgLoading = navigator.onLine !== false;
    _renderWeb();

    _bgSearchTimer = setTimeout(function(){ _doWebSearch(_webQ); }, 400);
  }

  /** Explicit submit (Enter key / Search button) — skips debounce, records history. */
  function _submitWebSearch(q) {
    _webQ = q || '';
    clearTimeout(_bgSearchTimer);
    if (_webQ.trim().length >= 2) {
      _webHistory = _histAdd(_webHistory, LIB_HIST_KEY_WEB, _webQ.trim());
      _renderWebHistory();
    }
    _doWebSearch(_webQ);
  }

  function _doWebSearch(q) {
    _bgCurrentQuery    = q;
    _bgExternalResults = [];
    _bgPubMedPage      = 1;
    _bgOAPage          = 1;
    ...
    if (!_bgLoading) {
      _toast('Offline — Web Library requires an internet connection', 'info');
      return;
    }
    ...
    Promise.all([
      ...
      _glUnifiedSearch(q, 1)
        .catch(function(){ return { results: [], total: 0 }; }),
      _rsFrontiersSearch(q, 1, _rsYearFrom, _rsYearTo)
        .catch(function(){ return { results: [], total: 0 }; }),
      _rsElsevierSearch(q, 1, _rsYearFrom, _rsYearTo)
        .catch(function(){ return { results: [], total: 0 }; })
    ]).then(function(arrs) {
  ...
  function _loadMoreExternal() {
    ...
    if (_bgHasMoreFR) {
      _bgFRPage++;
      _rsFrontiersSearch(q, _bgFRPage, _rsYearFrom, _rsYearTo)
        .then(function(d) {
          _bgFRTotal   = d.total;
    ...
    if (_bgHasMoreEL) {
      _bgELPage++;
      _rsElsevierSearch(q, _bgELPage, _rsYearFrom, _rsYearTo)
        .then(function(d) {
          _bgELTotal   = d.total;
      ...
      return '<div class="lib-my-card" data-lib-id="'+r.id+'">' +
        '<div class="lib-my-head">' +
          ...
          '<div style="flex:1;min-width:0">' +
            ...
            '<div style="font-family:var(--mono);font-size:11px;color:var(--text-muted);margin-top:2px">'+
    ...
    _unsubBrowse = awclient.subscribe(channel, function(response) {
      ...
      if (isUpdate) {
        ...
        if (normalised.status !== 'approved') {
          ...
          _sqClearCache();
          if (!_searchQ || !_searchQ.trim()) _renderLocal();
        }
        return;
      }
  ...
  function _openResource(resourceId) {
    ...
    meta.innerHTML =
      ...
      '</div>'+
      '<div style="font-family:var(--mono);font-size:11px;color:var(--text-muted);margin-bottom:10px;line-height:1.7">'+
      ...
      .then(function(data) {
        ...
        return fetch(summaryUrl)
          ...
          .then(function(sd) {
            ...
            }).filter(function(r) { return _yearInRange(r.year, yearFrom, yearTo); });
  ...
  /* ── _rsSearch / pagination removed — replaced by _webSearch / _loadMoreExternal ── */
  ...
  function _glCardHTML(r, idx) {
    ...
    var abstractHtml = hasAbstract
      ? '<div class="rs-result-abstract" id="gl-abs-' + idx + '">' + _esc(r.abstract) + '</div>' +
        '<button class="rs-act-btn" style="font-size:11px;padding:3px 8px" ' +
   ...
   * ────────────────────────────────────────────────────────────────────────── */
  function _rsElsevierSearch(query, page, yearFrom, yearTo) {
    if (!query) return Promise.resolve({ results: [], total: 0 });
    ...
    /* ── Year-range clause (Scopus supports PUBYEAR natively) ── */
    var pubYearClause = '';
    if (yearFrom) pubYearClause += ' AND PUBYEAR > ' + (parseInt(yearFrom, 10) - 1);
    if (yearTo)   pubYearClause += ' AND PUBYEAR < ' + (parseInt(yearTo, 10) + 1);
    ...
    return Promise.all([scopusPromise, sdPromise]).then(function(both) {
      ...
      /* Precise year filter — guarantees correctness even though Scopus
         already narrows server-side via PUBYEAR; ScienceDirect doesn't
         support an equivalent param, so this is the only enforcement
         point for it. */
      merged = merged.filter(function(r) { return _yearInRange(r.year, yearFrom, yearTo); });
  ...
  /* ── Web Library filter helpers ── */
  function _reRunWebSearch() {
    if (_webQ && _webQ.trim().length >= 2) _doWebSearch(_webQ);
  }

  function _toggleWebOpenAccess() {
    _rsOpenAccess = !_rsOpenAccess;
    var chip = document.getElementById('lib-web-oa-chip');
    if (chip) chip.classList.toggle('active', _rsOpenAccess);
    _reRunWebSearch();
  }

  function _toggleWebMalawiOnly() {
    _webMalawiOnly = !_webMalawiOnly;
    var chip = document.getElementById('lib-web-mw-chip');
    if (chip) chip.classList.toggle('active', _webMalawiOnly);
    _renderWeb();   // purely a render-time filter — no refetch needed
  }
  ...
  function _switchPanel(panel) {
    ...
    if (panel === 'local')     { _loadApproved(); _renderLocalHistory(); }
    if (panel === 'web')       { _renderWeb(); _renderWebHistory(); }
  ...
  function _buildYearOptions(placeholder) {
    var nowYear = new Date().getFullYear();
    var opts = '<option value="">'+placeholder+'</option>';
    for (var y = nowYear; y >= 1990; y--) {
    ...
    if (!document.getElementById('tab-library')) {
      ...
      wrap.innerHTML =
        ...
        '<nav class="lib-subnav" role="navigation" aria-label="Library navigation">'+
          '<button class="lib-sntab active" data-lpanel="local"    onclick="LibraryModule.switchPanel(\'local\')">📁 Local Library</button>'+
          '<button class="lib-sntab"        data-lpanel="web"      onclick="LibraryModule.switchPanel(\'web\')">🌐 Web Library</button>'+
        ...
        /* ── Local Library panel — Oasis-stored resources only ── */
        '<div class="lib-panel active" id="lib-panel-local">'+
          '<div class="lib-searchbar">'+
            '<span class="lib-search-icon">📁</span>'+
            '<input type="text" id="lib-local-q" '+
              'placeholder="Search Oasis-stored resources…" '+
              'autocomplete="off" spellcheck="false" '+
              'oninput="LibraryModule.onLocalSearch(this.value)" '+
              'onkeydown="if(event.key===\'Enter\')LibraryModule.submitLocalSearch(this.value)">'+
            '<button class="lib-search-btn" onclick="LibraryModule.submitLocalSearch(document.getElementById(\'lib-local-q\').value)">Search</button>'+
          '</div>'+
          /* Recent search history — Local Library */
          '<div id="lib-local-hist"></div>'+
          ...
          '<div class="lib-sec-hdr" style="margin-top:4px">'+
            '<div class="lib-sec-title">Oasis Library</div>'+
            '<div class="lib-badge" id="lib-local-count">0</div>'+
          '</div>'+
          '<div id="lib-local-cards" class="lib-cards"><div class="lib-spin"></div></div>'+
        '</div>'+

        /* ── Web Library panel — internet / external sources only ── */
        '<div class="lib-panel" id="lib-panel-web">'+
          '<div class="lib-searchbar">'+
            '<span class="lib-search-icon">🌐</span>'+
            '<input type="text" id="lib-web-q" '+
              'placeholder="Search WHO, ASPEN, ESPEN, NICE, PubMed…" '+
              'autocomplete="off" spellcheck="false" '+
              'oninput="LibraryModule.onWebSearch(this.value)" '+
              'onkeydown="if(event.key===\'Enter\')LibraryModule.submitWebSearch(this.value)">'+
            '<button class="lib-search-btn" onclick="LibraryModule.submitWebSearch(document.getElementById(\'lib-web-q\').value)">Search</button>'+
          '</div>'+
          /* Recent search history — Web Library */
          '<div id="lib-web-hist"></div>'+
          /* Source / year / open-access filters */
          '<div class="rs-filter-row">'+
            '<select class="rs-filter-sel" id="lib-web-yearfrom" onchange="LibraryModule.rsSetYearFrom(this.value);LibraryModule.reRunWebSearch()">'+
              _buildYearOptions('From year')+
            '</select>'+
            '<select class="rs-filter-sel" id="lib-web-yearto" onchange="LibraryModule.rsSetYearTo(this.value);LibraryModule.reRunWebSearch()">'+
              _buildYearOptions('To year')+
            '</select>'+
          '</div>'+
          '<div class="lib-chips">'+
            '<button class="lib-chip" id="lib-web-oa-chip" onclick="LibraryModule.toggleWebOpenAccess()">🔓 Open Access</button>'+
            '<button class="lib-chip" id="lib-web-mw-chip" onclick="LibraryModule.toggleWebMalawiOnly()">🇲🇼 Malawi / Africa only</button>'+
          '</div>'+
          /* Results info bar */
          '<div class="lib-sec-hdr" style="margin-top:4px">'+
            '<div class="lib-sec-title" id="lib-web-results-label">Web Sources</div>'+
            '<div class="lib-badge" id="lib-web-count">0</div>'+
          '</div>'+
          ...
          '<div id="lib-web-cards" class="lib-cards"><div class="lib-spin"></div></div>'+
        ...
        '<div class="lib-panel" id="lib-panel-upload">'+
          '<div class="lib-form-card">'+
            ...
            '<div class="lib-row">'+
              '<label class="lib-lbl">Tags <em>*</em> <span style="font-weight:400;text-transform:none;font-size:11px;opacity:.7">(Type tag → press comma → press backspace to add · max 10)</span></label>'+
            ...
            '<div id="lib-prog-wrap" style="display:none">'+
              ...
              '<div id="lib-prog-lbl" style="font-family:var(--mono);font-size:11px;color:var(--text-muted);margin-top:5px;text-align:center"></div>'+
            '</div>'+

            '<button class="lib-submit" id="lib-submit-btn" onclick="LibraryModule.submitUpload()">↑ SUBMIT FOR REVIEW</button>'+

            '<div style="margin-top:10px;text-align:center;font-family:var(--mono);font-size:11px;color:var(--text-muted);line-height:1.55">'+
  ...
  function _init() {
    ...
    /* Load persisted search history for both sections */
    _localHistory = _histLoad(LIB_HIST_KEY_LOC);
    _webHistory   = _histLoad(LIB_HIST_KEY_WEB);
  ...
  window.LibraryModule = {
    ...
    /* ── Local Library (Oasis-stored resources) ── */
    onLocalSearch: function(val) {
      _localSearch(val || '');
    },
    submitLocalSearch: function(val) {
      _submitLocalSearch(val || '');
    },
    ...
    searchLocalFromHistory: function(q) {
      var input = document.getElementById('lib-local-q');
      if (input) input.value = q;
      _submitLocalSearch(q);
    },
    clearLocalHistory: function() {
      _localHistory = _histClear(LIB_HIST_KEY_LOC);
      _renderLocalHistory();
    },

    /* ── Web Library (internet / external sources) ── */
    onWebSearch: function(val) {
      _webQ = val || '';
      if (_webQ.trim().length >= 2) {
        _webSearch(_webQ);
      } else {
        // Clear query — reset Web Library to its empty state
        clearTimeout(_bgSearchTimer);
        _bgLoading         = false;
        _bgExternalResults = [];
        _bgHasMore         = { pubmed: false, oa: false };
    ...
    submitWebSearch: function(val) {
      _submitWebSearch(val || '');
    },
    searchWebFromHistory: function(q) {
      var input = document.getElementById('lib-web-q');
      if (input) input.value = q;
      _submitWebSearch(q);
    },
    clearWebHistory: function() {
      _webHistory = _histClear(LIB_HIST_KEY_WEB);
      _renderWebHistory();
    },
    toggleWebOpenAccess:  _toggleWebOpenAccess,
    toggleWebMalawiOnly:  _toggleWebMalawiOnly,
    reRunWebSearch:       _reRunWebSearch,
