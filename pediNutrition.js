...
/**
 * buildNutritionInterventionCard(N, gaDec, bwtG, phase, route, stress, sex)
 *
 * Renders a clinically styled "Nutrition Intervention" explanation card with
 * four NCP domains (ND / E / C / RC). The JS logic computes a rich structured
 * data object and passes it to the Groq-backed Oasis AI which writes the final
 * clinical narrative for each domain. A polished skeleton is displayed
 * immediately while the AI call is in-flight; content is swapped in on arrival.
 *
 * No AI mention, no "generated by" text — the card reads as authored by an
 * experienced neonatal dietitian.
 */
function buildNutritionInterventionCard(N, gaDec, bwtG, phase, route, stress, sex) {

  /* ── Unique card ID for DOM targeting ── */
  const cardId  = 'ni-card-' + Date.now();
  const ndId    = cardId + '-nd';
  const eId     = cardId + '-e';
  const cId     = cardId + '-c';
  const rcId    = cardId + '-rc';
  ...
  const BW_LABELS = {
    periviable: 'periviable (<500 g)', ELBW: 'extremely low birth weight (ELBW <1 000 g)',
    VLBW: 'very low birth weight (VLBW <1 500 g)', LBW: 'low birth weight (LBW <2 500 g)',
    normal: 'appropriate birth weight'
  };
  const PHASE_LABELS = { transition: 'Transition (Day 1–3)', stable: 'Stable Growth', catchup: 'Catch-Up Growth' };
  const ROUTE_LABELS = {
    trophic: 'Trophic / Minimal EN', partial: 'Partial EN + IV Dextrose',
    full_en: 'Full Enteral Nutrition', tpn: 'Full IV Nutrition (TPN)'
  };
  const STRESS_LABELS = {
    none: 'None', sepsis: 'Sepsis / Infection', surgery: 'Post-surgical',
    vent: 'Mechanically Ventilated', rop: 'CLD / BPD', nec: 'NEC / Bowel Disease', mbdp: 'MBDP'
  };

  const bwLabel    = BW_LABELS[N.bwCat]    || N.bwCat;
  const phaseLabel = PHASE_LABELS[phase]   || phase;
  const routeLabel = ROUTE_LABELS[route]   || route;
  const stressLabel= STRESS_LABELS[stress] || stress;
  const bwLabel2   = { periviable:'Periviable', ELBW:'ELBW', VLBW:'VLBW', LBW:'LBW', normal:'Normal BW' }[N.bwCat] || N.bwCat;

  const isELBW  = N.bwCat === 'ELBW' || N.bwCat === 'periviable';
  const isVLBW  = N.bwCat === 'VLBW';
  const isNEC   = stress === 'nec';
  const isMBDP  = stress === 'mbdp';
  const isTPN   = route  === 'tpn';
  const isRadiant = N.therm === 'radiant';

  /* ══════════════════════════════════════════════════════════════
     BUILD RICH STRUCTURED CONTEXT FOR THE AI PROMPT
     The JS engine already calculated all values — we pass them as
     a dense clinical fact sheet so the AI enriches (not invents).
  ══════════════════════════════════════════════════════════════ */
  function buildClinicalContext() {
    const lines = [
      '=== PRETERM INFANT — CALCULATED NUTRITION DATA ===',
      `Gestational Age: ${gaWk} weeks`,
      `Birth Weight: ${bwStr} (${bwLabel})`,
      `Current Weight: ${N.wtKg ? N.wtKg.toFixed(3) + ' kg' : '?'}`,
      `Clinical Phase: ${phaseLabel}`,
      `Feeding Route (selected): ${routeLabel}`,
      `Stress / Complication: ${stressLabel}`,
      `Thermal Environment: ${N.therm === 'radiant' ? 'Radiant warmer (↑ IWL ~20-30 mL/kg/day)' : 'Enclosed incubator'}`,
      '',
      '--- FEEDING ROUTE RATIONALE ---',
      route === 'trophic'  ? `TROPHIC / MINIMAL EN: Selected because this infant (GA ${gaWk} wk, ${bwLabel}) is in the very early postnatal period or has GI instability precluding meaningful enteral advancement. Trophic feeds at ≤${N.enStart} mL/kg/day prime the gut mucosa and stimulate secretory IgA and hormonal responses without imposing significant osmotic or metabolic load. Primary energy and glucose support is delivered entirely via ${N.dexSol} IV dextrose (GIR ${N.girTarget.lo}–${N.girTarget.hi} mg/kg/min). Protein and fat delivery at trophic volumes is negligible — protein deficit will accumulate until feeds advance to ≥${Math.round(N.enFull * 0.5)} mL/kg/day. Advance by ${N.enAdvance} mL/kg/day q12–24h as clinical tolerance permits toward full feeds target of ${N.enFull} mL/kg/day (estimated ${N.daysToFull}).` : '',
      route === 'partial'  ? `PARTIAL EN + IV DEXTROSE: This route is appropriate for GA ${gaWk} wk infants transitioning from trophic to full enteral feeds where GI function is establishing but not yet sufficient to meet full nutritional demands enterally. EN is provided at ${N.enVolTarget} mL/kg/day (${N.enVolDay} mL/day), delivering ${N.enEnergyKcalKg} kcal/kg/day and approximately ${N.ebmProtKg} g/kg/day protein (EBM). The remaining energy deficit is bridged by ${N.dexSol} IV dextrose at GIR ${N.girTarget.lo}–${N.girTarget.hi} mg/kg/min, providing ${N.dexKcalKgCalc || N.dexKcalKg} kcal/kg/day. IV dextrose must be reduced proportionally as EN volume advances every 12–24 hours, maintaining total fluid intake within target. Enteral advancement to ${N.enFull} mL/kg/day is the primary clinical goal to achieve full protein and fat provision — no IV amino acids or lipid are available in this Malawi NICU context.` : '',
      route === 'full_en'  ? `FULL ENTERAL NUTRITION: Appropriate for this infant as GI function is sufficiently established to meet all nutritional requirements via the enteral route. EN is provided at ${N.enVolTarget} mL/kg/day targeting ${N.enFull} mL/kg/day. EBM (expressed breast milk) is the gold standard substrate — it provides 65 kcal/100 mL and 1.0 g protein/100 mL in addition to immunological and neurodevelopmental benefits unavailable from formula. At current volume (${N.enVolTarget} mL/kg/day), EBM provides ${N.enEnergyKcalKg} kcal/kg/day and ${N.ebmProtKg} g/kg/day protein. Since protein target is ${N.protTarget.lo}–${N.protTarget.hi} g/kg/day (ESPGHAN 2022), ${N.protDeficitEbm > 0 ? 'a deficit of ' + N.protDeficitEbm + ' g/kg/day exists — human milk fortification (HMF) should be introduced at ≥40–100 mL/kg/day.' : 'the target is being met or closely approached.'} IV dextrose continues for glucose stability and should be weaned gradually as full EN consolidates.` : '',
      isTPN ? `FULL IV NUTRITION (TPN): Selected because enteral feeding is contraindicated or insufficient to meet nutritional requirements at this stage (GA ${gaWk} wk, ${stressLabel}). TPN provides all three macronutrients intravenously: amino acids advancing from ${N.tpnAAInitial ? N.tpnAAInitial.lo + '–' + N.tpnAAInitial.hi : '2–3'} to ${N.tpnAATarget ? N.tpnAATarget.lo + '–' + N.tpnAATarget.hi : '3.5–4.0'} g/kg/day (advance over Day 3–5); lipid emulsion advancing from ${N.tpnLipidInitial ? N.tpnLipidInitial.lo + '–' + N.tpnLipidInitial.hi : '2–3'} to ${N.tpnLipidTargetKg ? N.tpnLipidTargetKg.toFixed(1) : '3'} g/kg/day over 24–48h; dextrose titrated from GIR ${N.tpnGIRInitial ? N.tpnGIRInitial.lo + '–' + N.tpnGIRInitial.hi : '5–7'} advancing by ${N.tpnGIRIncrement ? N.tpnGIRIncrement.lo + '–' + N.tpnGIRIncrement.hi : '1–2'} mg/kg/min/day to max ${N.tpnGIRMax ? N.tpnGIRMax.lo + '–' + N.tpnGIRMax.hi : '11–12'} mg/kg/min. Trophic EBM feeds should be introduced alongside TPN at the earliest safe opportunity to protect gut mucosal integrity and support colonisation. Active PN weaning toward full EN should be planned from Day 3–5.` : '',
      '',
      '--- ENERGY ---',
      `Total Energy Target: 110-130 kcal/kg/day (ESPGHAN 2022)`,
      `EN Energy at current volume (${N.enVolTarget} mL/kg/day): ${N.enEnergyKcalKg} kcal/kg/day`,
      `IV Dextrose Energy: ${N.dexKcalKgCalc || N.dexKcalKg} kcal/kg/day (${N.dexSol}, GIR ${N.girMid ? N.girMid.toFixed(1) : '?'} mg/kg/min)`,
      `Total Delivered: ${N.totalEnergyDeliveredKg} kcal/kg/day — ${N.energyMet ? 'WITHIN target' : 'BELOW target (gap ~' + (N.energyGapKg||'?') + ' kcal/kg/day)'}`,
      `Full Feeds Energy (at ${N.enFull} mL/kg/day): ${N.enEnergyKcalKgFull} kcal/kg/day`,
      '',
      '--- PROTEIN ---',
      `Target: ${N.protTarget.lo}-${N.protTarget.hi} g/kg/day (ESPGHAN 2022)`,
      `With EBM at current volume: ${N.ebmProtKg} g/kg/day — deficit ${N.protDeficitEbm > 0 ? N.protDeficitEbm + ' g/kg/day (' + (N.deficitSeverityEbm ? N.deficitSeverityEbm.label : 'deficit') + ')' : 'none'}`,
      `With Lactogen 1 (std): ${N.lag1ProtKg} g/kg/day`,
      `7-day cumulative protein deficit (EBM): ${N.cumDeficitEbm} g/kg`,
      `Postnatal growth failure risk: ${N.deficitSeverityEbm && N.deficitSeverityEbm.alert ? 'HIGH — urgent action required' : 'Monitor closely'}`,
      '',
      '--- ENTERAL ADVANCEMENT PLAN ---',
      `Start: ${N.enStart} mL/kg/day — Advance: ${N.enAdvance} mL/kg/day q12-24h — Full feeds target: ${N.enFull} mL/kg/day`,
      `Estimated days to full feeds: ${N.daysToFull}`,
      `Current EN volume: ${N.enVolTarget} mL/kg/day = ${N.enVolDay} mL/day`,
      route === 'trophic' ? 'Trophic feeds: gut priming ONLY — no meaningful macronutrient delivery' : '',
      `EBM at full feeds (${N.enFull} mL/kg/day): ${N.enEnergyKcalKgFull} kcal/kg/day`,
      `Lactogen 1 (standard) protein at current volume: ${N.lag1ProtKg} g/kg/day`,
      `Lactogen 1 (concentrated) protein at current volume: ${N.lag1ConcProtKg} g/kg/day`,
      `Fat delivery via EBM at current volume: ${N.ebmFatKg} g/kg/day`,
      '',
      '--- DEXTROSE / IV GLUCOSE ---',
      `Solution: ${N.dexSol} — Target GIR: ${N.girTarget ? N.girTarget.lo + '-' + N.girTarget.hi : '?'} mg/kg/min`,
      `Dextrose rate: ${N.dexRateLo}–${N.dexRateHi} mL/hr — mid: ${N.dexRateMid} mL/hr = ${N.dexVolDay} mL/day`,
      `Energy from dextrose: ${N.dexKcalDay} kcal/day = ${N.dexKcalKg} kcal/kg/day`,
      isTPN ? `TPN Amino Acids: initial ${N.tpnAAInitial ? N.tpnAAInitial.lo + '-' + N.tpnAAInitial.hi : '2-3'} → target ${N.tpnAATarget ? N.tpnAATarget.lo + '-' + N.tpnAATarget.hi : '3.5-4'} g/kg/day` : 'No IV amino acids available — enteral is ONLY protein source',
      isTPN ? `TPN Lipid: initial ${N.tpnLipidInitial ? N.tpnLipidInitial.lo + '-' + N.tpnLipidInitial.hi : '2-3'} → target ${N.tpnLipidTargetKg ? N.tpnLipidTargetKg.toFixed(1) : '3'} g/kg/day (24h infusion)` : 'No IV lipid available — enteral is ONLY fat source',
      '',
      '--- FLUID BALANCE & MANAGEMENT ---',
      `Total Fluid Target: ${N.fluidAdj.lo}–${N.fluidAdj.hi} mL/kg/day (mid: ${N.fluidTarget} mL/kg/day = ${N.fluidTotalMl} mL/day = ${N.fluidHrMl} mL/hr)`,
      `IV Dextrose: ${N.dexVolDay} mL/day (${N.dexRateMid} mL/hr) = ${N.ivPct}% of total fluid`,
      `Enteral volume: ${N.enVolDay} mL/day (${N.enVolTarget} mL/kg) = ${N.enPct}% of total fluid`,
      N.fluidIVAlert ? `FLUID ALERT: IV dextrose (${N.ivPct}%) currently EXCEEDS enteral volume (${N.enPct}%) in ${phaseLabel} phase — enteral feeds must be prioritised and advanced as tolerated.` : `Fluid balance is ${N.enPct >= N.ivPct ? 'enteral-predominant (favourable)' : 'IV-predominant (advance EN)'}`,
      isRadiant ? `Radiant warmer in use: +20–30 mL/kg/day added to fluid target for elevated insensible water loss (IWL). Total adjusted target: ${N.fluidAdj.lo}–${N.fluidAdj.hi} mL/kg/day. Transition to incubator as soon as clinically safe to reduce IWL.` : 'Enclosed incubator: standard fluid targets apply.',
      `Fluid weaning rule: reduce IV dextrose rate by ${Math.round(N.enAdvance * N.wtKg / 24 * 10) / 10} mL/hr for each ${N.enAdvance} mL/kg/day EN advancement.`,
      '',
      '--- ELECTROLYTES — TARGETS & MANAGEMENT ---',
      `Sodium (Na⁺): ${N.elec.na.lo}–${N.elec.na.hi} mEq/kg/day — restrict Day 1–2 (prerenal state); add to dextrose/PN once adequate urine output established. Monitor for hypernatraemia from IWL on radiant warmer.`,
      `Potassium (K⁺): ${N.elec.k.lo}–${N.elec.k.hi} mEq/kg/day — withhold Day 1 in ELBW until confirmed adequate UO. Add to dextrose bag q12h thereafter.`,
      `Calcium: ${N.elec.ca.lo}–${N.elec.ca.hi} mmol/kg/day — give IV (calcium gluconate 10%) if oral route unavailable. Critical for cardiac function and bone mineralisation.`,
      `Phosphate: ${N.elec.phos.lo}–${N.elec.phos.hi} mmol/kg/day — high demand during rapid growth. Hypophosphataemia risk → MBDP. Add to PN bag; monitor serum PO4 3×/week.`,
      `Magnesium: ${N.elec.mg.lo}–${N.elec.mg.hi} mmol/kg/day — supplement via IV magnesium sulphate if hypomagnesaemia confirmed.`,
      isMBDP ? 'MBDP: Check ALP + PO4 q2 weeks from week 4; Ca:P mass ratio ≤1.8; HMF when EN ≥40 mL/kg/day; ALP >500 U/L + low PO4 → add supplemental Ca 20 mg/kg/day + P 10–20 mg/kg/day PO.' : '',
      '',
      '--- MICRONUTRIENTS ---',
      `Vitamin D: ${N.vit.vitD}`,
      `Iron: ${N.vit.iron}`,
      `Vitamin A: ${N.vit.vitA}`,
      `Folate: ${N.vit.folate}`,
      '',
      '--- MONITORING RECOMMENDATIONS ---',
      `Weight: daily — target ${(N.bwCat === 'ELBW' || N.bwCat === 'periviable') ? '15–20 g/kg/day' : '20–30 g/day'} (ESPGHAN 2022)`,
      'Length + HC: weekly on Fenton 2013 charts, adjusted for corrected age until 2 years',
      'Blood glucose: q1–2h Day 1–2 (target 2.5–10 mmol/L); treat hypoglycaemia <2.5 mmol/L with D10W 2 mL/kg bolus IV',
      'Electrolytes: 3×/week; ALP + PO4 monthly from week 4 for BW <1500 g',
      route !== 'tpn' ? `Enteral tolerance: assess before each bolus — hold if gastric residual >50% of feed volume or abdominal distension; advance EN ${N.enAdvance} mL/kg/day q12–24h toward ${N.enFull} mL/kg/day` : 'PN line: daily catheter infection screen; reassess GI function each shift; initiate EN as soon as clinically safe',
      'Escalate to neonatology if: weight loss over 3 days, glucose instability, clinical deterioration',
      '',
      '--- CLINICAL CONTEXT ---',
      isNEC    ? 'NEC ACTIVE: Complete bowel rest — zero protein/fat delivery. Document cumulative deficit.' : '',
      isELBW   ? `ELBW: Highest nutritional risk. No IV amino acids/lipid available. Protein deficit accumulating. EBM is critical.` : '',
      isVLBW   ? `VLBW: Very high risk. Protein target achievable only via early full EN. Advance feeds as fast as clinically safe.` : '',
      isRadiant ? 'Radiant warmer: Elevated IWL — adjust fluid targets; transition to incubator when stable.' : '',
      `Weeks to term: ~${N.weeksToTerm} weeks`,
      'Context: Malawi resource-limited NICU — no IV amino acids or IV lipid emulsion available.',
    ].filter(Boolean).join('\n');
  ...
  function domainShell(id, code, title, subtitle, icon, accentColor, accentRgb, initialText, isLoading) {
    ...
    <div style="margin-bottom:20px">
      <div style="display:flex;align-items:center;gap:12px;padding:10px 15px;
        background:linear-gradient(90deg,rgba(${accentRgb},0.11),rgba(${accentRgb},0.02));
        border-left:4px solid ${accentColor};border-radius:0 10px 10px 0;margin-bottom:11px">
        <div style="width:36px;height:36px;border-radius:9px;background:rgba(${accentRgb},0.14);
          border:1.5px solid rgba(${accentRgb},0.32);display:flex;align-items:center;
  ...
  const ANIM_STYLE_ID = 'ni-card-pulse-anim';

  /* ── Compose full card HTML ── */
  const cardHtml = `
  <style id="${ANIM_STYLE_ID}">
    @keyframes ni-pulse{0%,100%{opacity:0.3;transform:scale(0.8)}50%{opacity:1;transform:scale(1.2)}}
    @keyframes ni-fadein{from{opacity:0;transform:translateY(4px)}to{opacity:1;transform:translateY(0)}}
    .ni-domain-text p{animation:ni-fadein 0.5s ease forwards}
  </style>

  <div class="card" id="${cardId}" style="margin-bottom:18px;border-color:rgba(96,165,250,0.32);
    box-shadow:0 0 28px rgba(96,165,250,0.05),0 2px 14px rgba(0,0,0,0.17)">

    <!-- Header -->
    <div class="card-header" style="background:linear-gradient(135deg,rgba(96,165,250,0.09),rgba(96,165,250,0.02));
      border-bottom-color:rgba(96,165,250,0.18);padding:14px 18px">
    ...
    <div class="card-body" style="padding:18px 18px 14px">
      ...
      ${domainShell(ndId,'ND','Food and/or Nutrient Delivery',
        'Feeding route selection &amp; rationale · Progression · Fortification · Fluid strategy &amp; balance · Electrolyte management · Monitoring plan',
        '🫙','#60a5fa','96,165,250', fallbackND(), true)}

      ${domainShell(eId,'E','Nutrition Education',
        'Feed preparation · Tolerance monitoring · Fortification · Storage · Hygiene · Caregiver guidance',
        '📚','#34d399','52,211,153', fallbackE(), true)}

      ${domainShell(cId,'C','Nutrition Counseling',
        'Feeding progression · Growth monitoring · Tolerance expectations · Family support · Transition planning',
        '💬','#f59e0b','245,158,11', fallbackC(), true)}

      ${domainShell(rcId,'RC','Coordination of Nutrition Care',
        'MDT roles · Neonatology · Nursing · Lactation · Pharmacy · Biochemical &amp; growth monitoring',
        '🔗','#a78bfa','167,139,250', fallbackRC(), true)}
      ...
      <div style="padding:8px 12px;background:rgba(96,165,250,0.04);
        ...
        ASPEN Neonatal &amp; Pediatric Nutrition Support Guidelines 2021 ·
        Anderson DM. Krause &amp; Mahan's Food &amp; the Nutrition Care Process, 16th Ed., Ch.43 ·
        AAP COFN Nutritional Needs of the Preterm Infant 2020 · WHO KMC Evidence Update 2023
      </div>
  ...
  /* ══════════════════════════════════════════════════════════════
     ASYNC AI ENRICHMENT — called after card is in the DOM
  ══════════════════════════════════════════════════════════════ */
  function _enrichWithAI() {
    ...
    /* ── Domain ND ── */
    const ndPrompt = `You are a senior neonatal dietitian writing the "Food and/or Nutrient Delivery (ND)" section of a Nutrition Care Process intervention plan for a preterm infant. This section must be comprehensive — it now encompasses feeding route, fluid management, electrolyte considerations, and monitoring recommendations that would otherwise appear in separate cards.
...
Use professional clinical nutrition terminology. Each bullet must specify the responsible discipline and an actionable, measurable task. Do NOT mention AI. Do NOT use paragraphs or headings.`;
  ...
  /* Schedule enrichment after the card lands in the DOM */
...
const WHO_LMS = {
  ...
  // ══════════════════════════════════════════════════════════════
  // BMI-FOR-AGE (BMIAZ) — BOYS  61–228 months (5–19 years)
  // Source: WHO Growth Reference 2007 · de Onis et al.
  // Index: age in whole months (61–228)
  // ══════════════════════════════════════════════════════════════
  bmiaz_boys_5to19: [
    // [age_months,  L,        M,        S      ]
    [ 61,  -1.5279, 15.2854, 0.08067],
    [ 62,  -1.5426, 15.2580, 0.08101],
    [ 63,  -1.5560, 15.2333, 0.08133],
    [ 64,  -1.5682, 15.2114, 0.08164],
    [ 65,  -1.5793, 15.1921, 0.08193],
    [ 66,  -1.5893, 15.1752, 0.08221],
    [ 67,  -1.5983, 15.1607, 0.08247],
    [ 68,  -1.6063, 15.1484, 0.08272],
    [ 69,  -1.6133, 15.1381, 0.08295],
    [ 70,  -1.6195, 15.1298, 0.08317],
    [ 71,  -1.6248, 15.1233, 0.08338],
    [ 72,  -1.6294, 15.1185, 0.08357],
    [ 73,  -1.6332, 15.1153, 0.08375],
    [ 74,  -1.6363, 15.1135, 0.08392],
    [ 75,  -1.6388, 15.1131, 0.08408],
    [ 76,  -1.6407, 15.1140, 0.08423],
    [ 77,  -1.6420, 15.1160, 0.08436],
    [ 78,  -1.6429, 15.1191, 0.08449],
    [ 79,  -1.6432, 15.1233, 0.08461],
    [ 80,  -1.6431, 15.1283, 0.08472],
    [ 81,  -1.6426, 15.1342, 0.08482],
    [ 82,  -1.6418, 15.1409, 0.08492],
    [ 83,  -1.6406, 15.1484, 0.08501],
    [ 84,  -1.6391, 15.1566, 0.08509],
    [ 85,  -1.6373, 15.1654, 0.08517],
    [ 86,  -1.6352, 15.1748, 0.08524],
    [ 87,  -1.6329, 15.1847, 0.08531],
    [ 88,  -1.6304, 15.1952, 0.08537],
    [ 89,  -1.6276, 15.2061, 0.08543],
    [ 90,  -1.6247, 15.2175, 0.08548],
    [ 91,  -1.6216, 15.2293, 0.08553],
    [ 92,  -1.6184, 15.2415, 0.08557],
    [ 93,  -1.6150, 15.2542, 0.08561],
    [ 94,  -1.6115, 15.2673, 0.08565],
    [ 95,  -1.6078, 15.2808, 0.08568],
    [ 96,  -1.6041, 15.2948, 0.08571],
    [ 97,  -1.6003, 15.3092, 0.08574],
    [ 98,  -1.5963, 15.3241, 0.08576],
    [ 99,  -1.5923, 15.3394, 0.08578],
    [100,  -1.5882, 15.3553, 0.08580],
    [101,  -1.5840, 15.3717, 0.08582],
    [102,  -1.5798, 15.3886, 0.08583],
    [103,  -1.5755, 15.4061, 0.08584],
    [104,  -1.5711, 15.4242, 0.08585],
    [105,  -1.5667, 15.4429, 0.08586],
    [106,  -1.5622, 15.4622, 0.08586],
    [107,  -1.5577, 15.4822, 0.08587],
    [108,  -1.5531, 15.5029, 0.08587],
    [109,  -1.5484, 15.5242, 0.08587],
    [110,  -1.5437, 15.5463, 0.08587],
    [111,  -1.5389, 15.5691, 0.08587],
    [112,  -1.5341, 15.5926, 0.08587],
    [113,  -1.5292, 15.6169, 0.08587],
    [114,  -1.5242, 15.6420, 0.08586],
    [115,  -1.5192, 15.6679, 0.08586],
    [116,  -1.5141, 15.6945, 0.08585],
    [117,  -1.5089, 15.7219, 0.08585],
    [118,  -1.5037, 15.7501, 0.08584],
    [119,  -1.4984, 15.7791, 0.08583],
    [120,  -1.4930, 15.8089, 0.08582],
    [121,  -1.4876, 15.8395, 0.08581],
    [122,  -1.4821, 15.8709, 0.08580],
    [123,  -1.4765, 15.9031, 0.08579],
    [124,  -1.4709, 15.9361, 0.08578],
    [125,  -1.4652, 15.9699, 0.08577],
    [126,  -1.4594, 16.0044, 0.08576],
    [127,  -1.4536, 16.0397, 0.08575],
    [128,  -1.4477, 16.0757, 0.08574],
    [129,  -1.4418, 16.1124, 0.08573],
    [130,  -1.4358, 16.1498, 0.08572],
    [131,  -1.4297, 16.1879, 0.08571],
    [132,  -1.4236, 16.2267, 0.08570],
    [133,  -1.4174, 16.2661, 0.08569],
    [134,  -1.4111, 16.3061, 0.08568],
    [135,  -1.4048, 16.3467, 0.08568],
    [136,  -1.3984, 16.3878, 0.08567],
    [137,  -1.3920, 16.4295, 0.08566],
    [138,  -1.3855, 16.4717, 0.08566],
    [139,  -1.3789, 16.5143, 0.08565],
    [140,  -1.3723, 16.5574, 0.08565],
    [141,  -1.3656, 16.6009, 0.08564],
    [142,  -1.3589, 16.6448, 0.08564],
    [143,  -1.3521, 16.6891, 0.08564],
    [144,  -1.3453, 16.7337, 0.08564],
    [145,  -1.3384, 16.7787, 0.08564],
    [146,  -1.3315, 16.8240, 0.08564],
    [147,  -1.3245, 16.8696, 0.08565],
    [148,  -1.3175, 16.9155, 0.08565],
    [149,  -1.3104, 16.9617, 0.08566],
    [150,  -1.3033, 17.0082, 0.08567],
    [151,  -1.2961, 17.0550, 0.08568],
    [152,  -1.2889, 17.1021, 0.08570],
    [153,  -1.2817, 17.1494, 0.08571],
    [154,  -1.2744, 17.1970, 0.08573],
    [155,  -1.2671, 17.2449, 0.08576],
    [156,  -1.2598, 17.2931, 0.08578],
    [157,  -1.2524, 17.3415, 0.08581],
    [158,  -1.2450, 17.3902, 0.08584],
    [159,  -1.2376, 17.4392, 0.08587],
    [160,  -1.2301, 17.4885, 0.08591],
    [161,  -1.2226, 17.5381, 0.08595],
    [162,  -1.2151, 17.5880, 0.08599],
    [163,  -1.2076, 17.6382, 0.08604],
    [164,  -1.2000, 17.6887, 0.08609],
    [165,  -1.1924, 17.7396, 0.08614],
    [166,  -1.1848, 17.7908, 0.08619],
    [167,  -1.1772, 17.8424, 0.08625],
    [168,  -1.1695, 17.8944, 0.08631],
    [169,  -1.1619, 17.9467, 0.08638],
    [170,  -1.1542, 17.9994, 0.08645],
    [171,  -1.1465, 18.0525, 0.08652],
    [172,  -1.1388, 18.1060, 0.08660],
    [173,  -1.1311, 18.1599, 0.08668],
    [174,  -1.1234, 18.2142, 0.08676],
    [175,  -1.1157, 18.2690, 0.08685],
    [176,  -1.1080, 18.3242, 0.08694],
    [177,  -1.1003, 18.3798, 0.08704],
    [178,  -1.0926, 18.4358, 0.08714],
    [179,  -1.0849, 18.4922, 0.08724],
    [180,  -1.0772, 18.5490, 0.08734],
    [181,  -1.0695, 18.6061, 0.08745],
    [182,  -1.0618, 18.6635, 0.08756],
    [183,  -1.0541, 18.7214, 0.08767],
    [184,  -1.0465, 18.7796, 0.08779],
    [185,  -1.0388, 18.8381, 0.08791],
    [186,  -1.0312, 18.8970, 0.08803],
    [187,  -1.0235, 18.9562, 0.08816],
    [188,  -1.0159, 19.0157, 0.08829],
    [189,  -1.0083, 19.0755, 0.08842],
    [190,  -1.0007, 19.1356, 0.08856],
    [191,  -0.9931, 19.1960, 0.08870],
    [192,  -0.9856, 19.2567, 0.08884],
    [193,  -0.9780, 19.3177, 0.08899],
    [194,  -0.9705, 19.3789, 0.08914],
    [195,  -0.9630, 19.4403, 0.08929],
    [196,  -0.9555, 19.5020, 0.08944],
    [197,  -0.9480, 19.5639, 0.08960],
    [198,  -0.9406, 19.6260, 0.08976],
    [199,  -0.9331, 19.6882, 0.08993],
    [200,  -0.9257, 19.7507, 0.09009],
    [201,  -0.9183, 19.8133, 0.09026],
    [202,  -0.9110, 19.8761, 0.09044],
    [203,  -0.9036, 19.9390, 0.09061],
    [204,  -0.8963, 20.0020, 0.09079],
    [205,  -0.8890, 20.0652, 0.09097],
    [206,  -0.8818, 20.1284, 0.09115],
    [207,  -0.8745, 20.1917, 0.09134],
    [208,  -0.8673, 20.2551, 0.09153],
    [209,  -0.8602, 20.3185, 0.09172],
    [210,  -0.8530, 20.3820, 0.09191],
    [211,  -0.8459, 20.4454, 0.09210],
    [212,  -0.8388, 20.5089, 0.09230],
    [213,  -0.8318, 20.5723, 0.09250],
    [214,  -0.8248, 20.6357, 0.09270],
    [215,  -0.8178, 20.6990, 0.09290],
    [216,  -0.8108, 20.7622, 0.09310],
    [217,  -0.8039, 20.8254, 0.09331],
    [218,  -0.7970, 20.8884, 0.09351],
    [219,  -0.7901, 20.9513, 0.09372],
    [220,  -0.7833, 21.0141, 0.09393],
    [221,  -0.7765, 21.0768, 0.09414],
    [222,  -0.7697, 21.1393, 0.09435],
    [223,  -0.7630, 21.2016, 0.09456],
    [224,  -0.7563, 21.2638, 0.09478],
    [225,  -0.7496, 21.3258, 0.09499],
    [226,  -0.7430, 21.3876, 0.09521],
    [227,  -0.7364, 21.4492, 0.09543],
    [228,  -0.7298, 21.5106, 0.09565],
  ],

  // ══════════════════════════════════════════════════════════════
  // BMI-FOR-AGE (BMIAZ) — GIRLS  61–228 months (5–19 years)
  // Source: WHO Growth Reference 2007 · de Onis et al.
  // Index: age in whole months (61–228)
  // ══════════════════════════════════════════════════════════════
  bmiaz_girls_5to19: [
    // [age_months,  L,        M,        S      ]
    [ 61,  -1.8700, 15.2342, 0.08477],
    [ 62,  -1.8849, 15.2023, 0.08517],
    [ 63,  -1.8985, 15.1728, 0.08554],
    [ 64,  -1.9109, 15.1456, 0.08590],
    [ 65,  -1.9221, 15.1207, 0.08624],
    [ 66,  -1.9323, 15.0979, 0.08657],
    [ 67,  -1.9414, 15.0771, 0.08688],
    [ 68,  -1.9496, 15.0582, 0.08718],
    [ 69,  -1.9568, 15.0411, 0.08746],
    [ 70,  -1.9631, 15.0258, 0.08773],
    [ 71,  -1.9686, 15.0121, 0.08798],
    [ 72,  -1.9733, 15.0000, 0.08822],
    [ 73,  -1.9773, 14.9893, 0.08845],
    [ 74,  -1.9806, 14.9800, 0.08866],
    [ 75,  -1.9832, 14.9720, 0.08886],
    [ 76,  -1.9853, 14.9652, 0.08905],
    [ 77,  -1.9868, 14.9596, 0.08922],
    [ 78,  -1.9878, 14.9551, 0.08939],
    [ 79,  -1.9883, 14.9517, 0.08954],
    [ 80,  -1.9884, 14.9492, 0.08968],
    [ 81,  -1.9880, 14.9477, 0.08982],
    [ 82,  -1.9873, 14.9470, 0.08994],
    [ 83,  -1.9862, 14.9472, 0.09006],
    [ 84,  -1.9848, 14.9481, 0.09017],
    [ 85,  -1.9831, 14.9497, 0.09027],
    [ 86,  -1.9812, 14.9520, 0.09037],
    [ 87,  -1.9790, 14.9550, 0.09046],
    [ 88,  -1.9766, 14.9585, 0.09054],
    [ 89,  -1.9740, 14.9626, 0.09062],
    [ 90,  -1.9712, 14.9672, 0.09069],
    [ 91,  -1.9682, 14.9723, 0.09076],
    [ 92,  -1.9651, 14.9779, 0.09082],
    [ 93,  -1.9618, 14.9839, 0.09088],
    [ 94,  -1.9584, 14.9904, 0.09093],
    [ 95,  -1.9548, 14.9973, 0.09098],
    [ 96,  -1.9511, 15.0046, 0.09103],
    [ 97,  -1.9473, 15.0123, 0.09107],
    [ 98,  -1.9434, 15.0204, 0.09111],
    [ 99,  -1.9393, 15.0289, 0.09115],
    [100,  -1.9351, 15.0378, 0.09118],
    [101,  -1.9309, 15.0471, 0.09121],
    [102,  -1.9265, 15.0568, 0.09124],
    [103,  -1.9220, 15.0669, 0.09126],
    [104,  -1.9175, 15.0774, 0.09129],
    [105,  -1.9128, 15.0883, 0.09131],
    [106,  -1.9081, 15.0996, 0.09133],
    [107,  -1.9033, 15.1113, 0.09135],
    [108,  -1.8984, 15.1234, 0.09137],
    [109,  -1.8934, 15.1360, 0.09139],
    [110,  -1.8884, 15.1490, 0.09140],
    [111,  -1.8832, 15.1624, 0.09142],
    [112,  -1.8780, 15.1763, 0.09143],
    [113,  -1.8727, 15.1907, 0.09145],
    [114,  -1.8674, 15.2055, 0.09146],
    [115,  -1.8619, 15.2208, 0.09148],
    [116,  -1.8564, 15.2366, 0.09149],
    [117,  -1.8508, 15.2530, 0.09150],
    [118,  -1.8452, 15.2698, 0.09152],
    [119,  -1.8394, 15.2872, 0.09153],
    [120,  -1.8336, 15.3051, 0.09155],
    [121,  -1.8278, 15.3235, 0.09156],
    [122,  -1.8218, 15.3425, 0.09158],
    [123,  -1.8158, 15.3620, 0.09159],
    [124,  -1.8097, 15.3821, 0.09161],
    [125,  -1.8036, 15.4027, 0.09163],
    [126,  -1.7974, 15.4239, 0.09165],
    [127,  -1.7911, 15.4457, 0.09167],
    [128,  -1.7848, 15.4680, 0.09170],
    [129,  -1.7784, 15.4909, 0.09172],
    [130,  -1.7719, 15.5143, 0.09175],
    [131,  -1.7654, 15.5382, 0.09178],
    [132,  -1.7588, 15.5627, 0.09181],
    [133,  -1.7522, 15.5878, 0.09184],
    [134,  -1.7455, 15.6134, 0.09187],
    [135,  -1.7388, 15.6395, 0.09191],
    [136,  -1.7320, 15.6661, 0.09195],
    [137,  -1.7252, 15.6933, 0.09199],
    [138,  -1.7183, 15.7210, 0.09203],
    [139,  -1.7114, 15.7492, 0.09208],
    [140,  -1.7044, 15.7780, 0.09213],
    [141,  -1.6974, 15.8072, 0.09218],
    [142,  -1.6903, 15.8370, 0.09223],
    [143,  -1.6832, 15.8673, 0.09229],
    [144,  -1.6761, 15.8981, 0.09235],
    [145,  -1.6689, 15.9294, 0.09241],
    [146,  -1.6617, 15.9612, 0.09248],
    [147,  -1.6545, 15.9935, 0.09255],
    [148,  -1.6472, 16.0263, 0.09262],
    [149,  -1.6399, 16.0595, 0.09270],
    [150,  -1.6326, 16.0932, 0.09278],
    [151,  -1.6252, 16.1273, 0.09286],
    [152,  -1.6179, 16.1619, 0.09295],
    [153,  -1.6105, 16.1969, 0.09304],
    [154,  -1.6031, 16.2323, 0.09313],
    [155,  -1.5956, 16.2681, 0.09323],
    [156,  -1.5882, 16.3043, 0.09333],
    [157,  -1.5807, 16.3408, 0.09344],
    [158,  -1.5733, 16.3778, 0.09354],
    [159,  -1.5658, 16.4151, 0.09366],
    [160,  -1.5583, 16.4528, 0.09377],
    [161,  -1.5508, 16.4908, 0.09389],
    [162,  -1.5433, 16.5292, 0.09401],
    [163,  -1.5358, 16.5679, 0.09414],
    [164,  -1.5283, 16.6069, 0.09427],
    [165,  -1.5208, 16.6463, 0.09440],
    [166,  -1.5133, 16.6860, 0.09454],
    [167,  -1.5058, 16.7260, 0.09468],
    [168,  -1.4983, 16.7663, 0.09483],
    [169,  -1.4908, 16.8069, 0.09498],
    [170,  -1.4833, 16.8478, 0.09513],
    [171,  -1.4759, 16.8890, 0.09529],
    [172,  -1.4684, 16.9304, 0.09545],
    [173,  -1.4610, 16.9721, 0.09561],
    [174,  -1.4535, 17.0141, 0.09578],
    [175,  -1.4461, 17.0564, 0.09595],
    [176,  -1.4387, 17.0989, 0.09613],
    [177,  -1.4313, 17.1416, 0.09631],
    [178,  -1.4240, 17.1846, 0.09649],
    [179,  -1.4166, 17.2278, 0.09668],
    [180,  -1.4093, 17.2712, 0.09687],
    [181,  -1.4020, 17.3149, 0.09706],
    [182,  -1.3948, 17.3588, 0.09726],
    [183,  -1.3875, 17.4029, 0.09746],
    [184,  -1.3803, 17.4472, 0.09766],
    [185,  -1.3731, 17.4917, 0.09787],
    [186,  -1.3660, 17.5364, 0.09808],
    [187,  -1.3589, 17.5813, 0.09829],
    [188,  -1.3518, 17.6264, 0.09851],
    [189,  -1.3447, 17.6716, 0.09873],
    [190,  -1.3377, 17.7170, 0.09895],
    [191,  -1.3307, 17.7626, 0.09918],
    [192,  -1.3237, 17.8083, 0.09941],
    [193,  -1.3168, 17.8542, 0.09964],
    [194,  -1.3099, 17.9002, 0.09987],
    [195,  -1.3030, 17.9463, 0.10011],
    [196,  -1.2962, 17.9926, 0.10035],
    [197,  -1.2894, 18.0390, 0.10060],
    [198,  -1.2826, 18.0855, 0.10084],
    [199,  -1.2759, 18.1321, 0.10109],
    [200,  -1.2692, 18.1788, 0.10134],
    [201,  -1.2625, 18.2256, 0.10160],
    [202,  -1.2559, 18.2724, 0.10185],
    [203,  -1.2493, 18.3193, 0.10211],
    [204,  -1.2428, 18.3663, 0.10238],
    [205,  -1.2363, 18.4133, 0.10264],
    [206,  -1.2298, 18.4603, 0.10291],
    [207,  -1.2234, 18.5073, 0.10318],
    [208,  -1.2170, 18.5544, 0.10346],
    [209,  -1.2106, 18.6014, 0.10373],
    [210,  -1.2043, 18.6484, 0.10401],
    [211,  -1.1980, 18.6953, 0.10429],
    [212,  -1.1918, 18.7422, 0.10457],
    [213,  -1.1856, 18.7891, 0.10486],
    [214,  -1.1794, 18.8358, 0.10514],
    [215,  -1.1733, 18.8825, 0.10543],
    [216,  -1.1672, 18.9291, 0.10572],
    [217,  -1.1612, 18.9756, 0.10601],
    [218,  -1.1552, 19.0219, 0.10631],
    [219,  -1.1492, 19.0682, 0.10660],
    [220,  -1.1433, 19.1142, 0.10690],
    [221,  -1.1374, 19.1602, 0.10720],
    [222,  -1.1316, 19.2059, 0.10750],
    [223,  -1.1258, 19.2515, 0.10780],
    [224,  -1.1200, 19.2969, 0.10810],
    [225,  -1.1143, 19.3421, 0.10840],
    [226,  -1.1086, 19.3871, 0.10871],
    [227,  -1.1030, 19.4319, 0.10901],
    [228,  -1.0974, 19.4765, 0.10932],
  ],
 ...
 * BMI-for-age Z-score.
 *   0–60 months  → WHO 2006 Child Growth Standards (bmiaz_boys / bmiaz_girls)
 *   61–228 months → WHO 2007 Growth Reference 5–19yr (bmiaz_boys_5to19 / bmiaz_girls_5to19)
 * Both table sets share the same LMS interpolation approach.
 */
function calculateBMIAZ(bmi, age_months, sex) {
  let table;
  if (age_months > 60) {
    // WHO 2007 (5–19 yr) — used for all children 61–228 months (5–19 yr)
    table = sex === 'male' ? WHO_LMS.bmiaz_boys_5to19 : WHO_LMS.bmiaz_girls_5to19;
    if (!table) return { error: 'WHO 2007 BMI-for-age table (5–19yr) not loaded' };
    const lms = whoInterpolateLMS(table, age_months);
    if (!lms) return { error: `Age ${age_months} months out of WHO 2007 BMI-for-age range (61–228 months)` };
    const z = whoCalcZ(bmi, lms.L, lms.M, lms.S);
    return { z, percentile: zToPercentile(z), median: lms.M, lms, reference: 'WHO 2007' };
  } else {
    // WHO 2006 (0–5 yr)
    table = sex === 'male' ? WHO_LMS.bmiaz_boys : WHO_LMS.bmiaz_girls;
    const lms = whoInterpolateLMS(table, age_months);
    if (!lms) return { error: `Age ${age_months} months out of BMI-for-age range (0–60 months)` };
    const z = whoCalcZ(bmi, lms.L, lms.M, lms.S);
    return { z, percentile: zToPercentile(z), median: lms.M, lms, reference: 'WHO 2006' };
  }
}
...
const PediClassification = (() => {
  ...
  const MUAC_THRESHOLDS = {
    ...
    '180-216':{ sam: 160,   mam: 185,   note: 'Malawi CMAM 2016 — 16–17 years (late adolescent; adult thresholds apply at 18yr)' },
  };
  ...
  function _getMuacBand(ageMo) {
    ...
    if (ageMo < 180)  return MUAC_THRESHOLDS['120-180'];
    return                   MUAC_THRESHOLDS['180-216']; // 15–17yr (180–215 months)
  }
