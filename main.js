...
(function hpSlideshowInit() {
  let current = 0;
  let timer = null;
  const INTERVAL = 4500;

  function getSlides() { return document.querySelectorAll('.hp-slide'); }
  function getDots()   { return document.querySelectorAll('.hp-slide-dot'); }
  ...
  function buildDots() {
    const container = document.getElementById('hp-slide-dots');
    if (!container) return;
    const slides = getSlides();
    container.innerHTML = '';
    slides.forEach((_, i) => {
      const btn = document.createElement('button');
      btn.className = 'hp-slide-dot' + (i === 0 ? ' active' : '');
  ...
  function attachSwipe() {
    const el = document.getElementById('hp-slideshow');
    if (!el) return;
    let startX = 0;
    el.addEventListener('touchstart', e => { startX = e.touches[0].clientX; }, { passive: true });
    el.addEventListener('touchend', e => {
      const dx = e.changedTouches[0].clientX - startX;
      if (Math.abs(dx) > 40) { dx < 0 ? hpSlideNext() : hpSlidePrev(); resetTimer(); }
    }, { passive: true });
  }
  ...
  window.hpSlideNext = function () { goTo(current + 1); resetTimer(); };
  window.hpSlidePrev = function () { goTo(current - 1); resetTimer(); };
})();
// ═════════════════════════════════════════════════════════
