...
// ENTERAL FORMULA DATABASE
// ─────────────────────────────────────────────────────────────
// No longer hardcoded. Populated at runtime from the Chakudya Nutrition
// Registry's GET /formulas endpoint via NTFoodSearch (foodSearch.js), which
// already wholesale-syncs that registry into its own offline-first
// IndexedDB cache ("OasisCNRCache", store "formulas") every 15 minutes
// while online — this file does not open a second IndexedDB connection or
// duplicate that fetch/cache logic, it just reads from it.
//
// IMPORTANT — data-quality difference from the old hardcoded table:
// GET /formulas is CNR's community-submitted formula registry, not a
// clinically curated one. The retired hardcoded ENTERAL_DB above had a
// hand-written `note` field per product (indications, contraindications,
// flavours, Halaal/Kosher status, mechanism-of-action detail, "not
// suitable <3 yrs", etc.) sourced from ASPEN/ESPEN guidelines and
// manufacturer SPCs. The /formulas schema has no equivalent field — rows
// are typically just name/category/route/kcal-per-mL/macros/osmolality.
// `note` is preserved defensively in foodSearch.js's normalizer in case a
// notes/description column is ever added server-side, but until then it
// will be an empty string here, NOT the rich clinical annotation clinicians
// may be used to. The Formula Reference tool below still works (search,
// filter, sort, highlight cards) — it just won't show that prose until
// CNR's dataset carries it.
//
// Mutated in place (never reassigned) so every existing reference
// (`ENTERAL_DB.filter(...)`, `ENTERAL_DB.find(...)`, `ENTERAL_DB.length`,
// `ENTERAL_DB.indexOf(...)`) keeps working unchanged once formulas arrive.
// Until the first sync completes it's simply an empty list (no errors).
// ══════════════════════════════════════════════════════════════
const ENTERAL_DB = [];

/** Unified /formulas object (see foodSearch.js's _chakudyaFormulaRowToUnified)
 *  → ENTERAL_DB row shape (name, cat, route, kcalML, pro, cho, fat, osm,
 *  fibre, note). unified.kcal is per-100mL; ENTERAL_DB's kcalML is per-mL. */
function _unifiedFormulaToEnteralRow(u) {
  return {
    name:  u.name,
    cat:   u.cat || 'Enteral Formula',
    route: u.route || 'Unspecified',
    kcalML: u.kcal != null ? +(u.kcal / 100).toFixed(2) : 0,
    pro:   u.pro   ?? 0,
    cho:   u.cho   ?? 0,
    fat:   u.fat   ?? 0,
    osm:   u.osm   ?? null,
    fibre: u.fibre ?? u.fiber ?? 0,
    // Never null/undefined — enRender() does f.note.toLowerCase() unguarded.
    note:  u.note || '',
  };
}

function _refreshEnteralDbFromCNR() {
  if (typeof NTFoodSearch === 'undefined' || typeof NTFoodSearch.getAllFormulas !== 'function') return;
  const rows = NTFoodSearch.getAllFormulas().map(_unifiedFormulaToEnteralRow);
  ENTERAL_DB.length = 0;
  ENTERAL_DB.push(...rows);
  // Re-render the Formula Reference tool if it's already been opened this
  // session, so a background sync doesn't leave a stale list on screen.
  if (typeof enInitialized !== 'undefined' && enInitialized) {
    try { enRender(); enRenderHighlights(); } catch (_e) { /* tool not on screen */ }
  }
}

(function _initEnteralDbLoader() {
  if (typeof window === 'undefined') return;
  if (typeof NTFoodSearch !== 'undefined' && NTFoodSearch.ready) {
    NTFoodSearch.ready().then(_refreshEnteralDbFromCNR);
  }
  window.addEventListener('oasis:formulas-updated', _refreshEnteralDbFromCNR);
})();
